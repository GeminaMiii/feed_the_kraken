...
| 双面地图（快/长） | R P4/P6 + 用户照片 | data/maps.ts（快图22格逐格转录） | setup.test 快航程结构/距离 | ✅（拓扑建议实体板复核） |
...
| 项 | 影响 | 状态 |
|---|---|---|
| 快航程地图 | 已按用户提供的官方照片完成转录，19张牌启用，auto=5-7人快图 | ✅ |
| 长航程地图出口表逐格核验 | 极小概率个别出口与实体板不符 | 🟡 图标分布已核对，拓扑来自社区逆向 |
